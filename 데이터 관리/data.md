...
데이터가 적고 클래스가 불균형할 때, **SMOTE**로 소수 클래스 샘플 사이를 보간하여 합성 데이터를 생성합니다.
**XGBoost**(적은 데이터에 강한 앙상블 모델)와 조합하면 양성 재현율이 60% → 85%까지 개선됩니다.

> 증강은 **학습 데이터에만** 적용. 테스트 데이터는 절대 증강하지 않습니다.

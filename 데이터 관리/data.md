# 데이터 기초 개론

> ⏱ 15분 | 참조 자료

AI/머신러닝에서 **"데이터"**는 모델이 학습하고 평가받는 재료입니다.
아무리 좋은 알고리즘이라도 데이터를 제대로 다루지 못하면 의미 있는 결과를 낼 수 없습니다.
이 장에서는 데이터를 다루는 가장 기초적인 개념 세 가지를 다룹니다.

---

## 1. 학습 데이터와 테스트 데이터의 분리

### 1-1. 왜 분리하는가?

모델은 주어진 데이터의 패턴을 학습합니다.
그런데 학습에 사용한 데이터로 성능을 평가하면, 모델이 정답을 **"외웠는지"** 아니면 진짜 **"이해했는지"** 구분할 수 없습니다.

> 시험 문제를 미리 알고 본 시험에서 100점을 받는 것과 같습니다.

따라서 데이터를 반드시 학습용(Training Set)과 평가용(Test Set)으로 나눠야 합니다.

### 1-2. 대표적인 데이터 분리 기법

분리 방식에 따라 모델 정확도의 신뢰성이 달라집니다. 잘못 분리하면 학습 시 93%인데 실제 배포 후 78%가 될 수 있습니다.

#### 1-2-1. Hold-Out (단순 분리)

![Hold-Out](https://algotrading101.com/learn/wp-content/uploads/2020/06/training-validation-test-data-set.png)

#### 1-2-2. K-Fold 교차 검증 (K=5 예시)

![K-Fold CV](https://imghub.insilicogen.com/media/photos/cv.png)

> 매 회차마다 다른 Fold가 테스트 → 5번 반복 → 평균 성능 = 안정적 평가

| 기법 | 핵심 개념 | 적합한 상황 | 정확도 신뢰도 |
|------|-----------|------------|:------------:|
| **Hold-Out** | 데이터를 한 번만 학습/테스트로 분리 | 데이터가 충분히 많을 때 (10만+) | 보통 |
| **K-Fold CV** | K등분 → K번 반복 학습/평가 → 평균 | 데이터가 적을 때, 안정적 평가 필요 시 | 높음 |
| **Stratified K-Fold** | K-Fold + 각 Fold에서 클래스 비율 유지 | 클래스 불균형 분류 문제 (가장 권장) | 매우 높음 |
| **Time Series Split** | 시간 순서대로 과거→학습, 미래→테스트 | 주가, 날씨, 로그 등 시계열 데이터 | 높음 |

### 1-3. 데이터가 부족할 때 — 데이터 증강(Data Augmentation)

데이터가 적거나 클래스가 불균형할 때, 기존 데이터를 기반으로 합성 데이터를 생성하여 학습량을 늘립니다.
증강은 **학습 데이터에만** 적용하며, 테스트 데이터는 절대 증강하지 않습니다.

#### 1-3-1. SMOTE 원리

![SMOTE](https://imghub.insilicogen.com/media/photos/SMOTE_OPUxJ6w.png)


#### 1-3-2. 이미지 증강 예시

![이미지 증강](https://www.ibm.com/adobe/dynamicmedia/deliver/dm-aid--eaee14ad-c604-487e-a2c0-a8aa7d118fc2/data-augmentation-image-augment.png?preferwebp=true)

> 원본 1장 → 증강으로 4~10장의 변형 데이터 생성 가능

| 기법 | 대상 | 핵심 원리 |
|------|------|-----------|
| **SMOTE** | 테이블 데이터 | 소수 클래스 샘플 사이를 보간하여 합성 데이터 생성 |
| **이미지 증강** | 이미지 데이터 | 회전, 반전, 크기 조절, 색상 변환 등으로 변형 데이터 생성 |
| **XGBoost + SMOTE** | 불균형 분류 | 적은 데이터에 강한 앙상블 모델 + 증강 조합 → 재현율 60%→85% 개선 |

---

## 2. 오버피팅(과적합) 문제

### 2-1. 오버피팅이란?

모델이 학습 데이터에 **지나치게 맞춰져서**, 학습 데이터에서는 정확도가 높지만 새로운 데이터에서는 성능이 급격히 떨어지는 현상입니다.
모델이 데이터의 일반적인 패턴이 아니라 **노이즈(잡음)까지 외워버린 상태**입니다.

#### 2-1-1. Learning Curve (학습 곡선) — 오버피팅 판별의 핵심

![오버피팅 Learning Curve](https://static.wikidocs.net/images/page/152777/overfit.JPG)

### 오버피팅 vs 언더피팅

| 상태 | 학습 정확도 | 테스트 정확도 | 설명 |
|------|:-----------:|:------------:|------|
| **언더피팅** | 낮음 | 낮음 | 모델이 너무 단순, 패턴 포착 실패 |
| **적절한 학습** | 높음 | 높음 | 일반적 패턴 포착, 새 데이터에도 잘 작동 |
| **오버피팅** | 매우 높음 | 낮음 | 노이즈까지 외워버린 상태 |

### 2-2. Feature 간 다중공선성 (Multicollinearity)

![다중공선성](https://oopy.lazyrockets.com/api/v2/notion/image?src=https%3A%2F%2Fs3-us-west-2.amazonaws.com%2Fsecure.notion-static.com%2Fd9ea552d-e7fc-4083-8f89-b3c2b69962fc%2FUntitled.png&blockId=56087be2-b720-4ef2-afd1-000edd49cbde)

X Feature 간 상관관계가 높으면(예: "키"와 "신발 사이즈"), 두 Feature가 거의 동일한 패턴을 띄기 때문에 모델이 어떤 Feature가 진짜 영향을 주는지 구분하지 못합니다. 결과적으로 가중치가 불안정해지고 학습에 지장이 생깁니다.

| 상황 | 정확도 | 이유 |
|------|:------:|------|
| Training/Test Set 평가 | 높음 (90%+) | 학습 데이터와 같은 분포 → 중복 Feature로도 잘 맞음 |
| 새로운 데이터 투입 | 급락 (70%↓) | 데이터 분포가 조금만 달라져도 불안정한 가중치가 흔들림 |

> 다중공선성이 있으면 학습/테스트에서는 성능이 잘 나오지만, 실제 새로운 데이터에서는 모델 강건성(Robustness)이 무너져 정확도가 크게 떨어집니다.

---

## 3. 데이터 타입별 변환 방법

### 3-1. 텍스트 데이터 → CSV / Pandas DataFrame

텍스트 데이터는 CSV 파일로 저장하고, Pandas의 DataFrame으로 변환하여 행/열 단위로 다룹니다.

![DataFrame](https://media.geeksforgeeks.org/wp-content/uploads/20251206163931071616/dataframe.webp)

> CSV → `pd.read_csv()` → DataFrame으로 변환하여 필터링, 정렬, 통계 등 데이터 조작 수행

#### 왜 DataFrame을 쓰는가?

DataFrame은 행(row) × 열(column) 기반의 **2차원 벡터 구조**입니다.
일반 문자열(varchar/str)로 데이터를 다루면 200만 건 처리에 수십 분이 걸리지만, DataFrame은 내부적으로 C 기반 연산을 사용해 **수 초 만에 처리**합니다.

| 항목 | varchar / str (일반 문자열) | Pandas DataFrame (2차원 벡터) |
|------|:------------:|:----------------:|
| **구조** | 단일 값 하나하나 | 행 × 열 테이블 전체를 한 덩어리로 관리 |
| **200만 건 필터링** | for문 반복 → 수십 분 | 벡터 연산 → 수 초 |
| **연산 방식** | Python이 한 줄씩 처리 (느림) | C/NumPy가 한꺼번에 처리 (빠름) |
| **메모리** | 값마다 개별 저장 (비효율) | 열 단위 연속 메모리 (효율적) |
| **예시** | `for row in data: if row > 10` | `df[df["col"] > 10]` (한 줄로 끝) |

> 데이터가 수만~수백만 건일 때 DataFrame을 쓰는 이유 = **속도**. 같은 작업이 수백 배 빠릅니다.

---

### 3-2. 이미지 데이터 → RGB 값 변환

컴퓨터는 이미지를 **픽셀 단위의 숫자 배열**로 인식합니다.
각 픽셀은 **RGB**(Red, Green, Blue) 세 채널의 값(`0~255`)으로 구성됩니다.

#### 이미지 구조

| 타입 | 배열 형태 | 설명 |
|------|-----------|------|
| **흑백** (Grayscale) | `(높이, 너비)` | 각 픽셀: `0`(검정) ~ `255`(흰색) |
| **컬러** (RGB) | `(높이, 너비, 3)` | 각 픽셀: `[R, G, B]` 예) `[255, 0, 0]` = 빨간색 |

#### 정규화 (Normalization)

픽셀 값을 `0~1` 범위로 변환하면 학습 속도와 안정성이 향상됩니다.

```python
img_normalized = img_array / 255.0    # 0~255 → 0.0~1.0
```

#### 이미지 크기 통일 (Resize)

모델 입력을 위해 모든 이미지 크기를 동일하게 맞춥니다.

```python
img_resized = img.resize((224, 224))
img_array = np.array(img_resized) / 255.0
```

---

## 4. 토크나이저 (Tokenizer)

### 4-1. 토크나이저란?

![토크나이저](https://devocean.sk.com/editorImg/2023/2/21/705f461a19db46207a8f895d8744a71d8922440f83e6a03f7c3393f37632cea6)

텍스트를 모델이 이해할 수 있는 최소 단위(토큰)로 쪼개는 과정입니다.
컴퓨터는 문자열을 그대로 이해하지 못하기 때문에, 텍스트를 숫자(토큰 ID)로 변환해야 합니다.

> "오늘 날씨가 좋다" → ["오늘", "날씨", "가", "좋", "다"] → [1024, 532, 18, 295, 7]

### 4-2. 토큰화 방식

| 방식 | 단위 | 예시 ("나는 학생입니다") | 특징 |
|------|------|------------------------|------|
| **Word (단어)** | 띄어쓰기/공백 | ["나는", "학생입니다"] | 단순하지만 어휘 사전이 커짐, 처음 보는 단어 처리 불가 |
| **Character (글자)** | 한 글자씩 | ["나", "는", "학", "생", "입", "니", "다"] | 사전은 작지만 토큰 수가 너무 많아 문맥 파악 어려움 |
| **Subword (서브워드)** | 자주 쓰는 조각 | ["나는", "학생", "입니다"] | 단어와 글자의 중간. 현재 가장 많이 사용 |

### 4-3. RVQ (Residual Vector Quantization) — 다차원 양자화

![RVQ](https://miro.medium.com/v2/resize:fit:1032/0*LOtjc3UB6DKQGft2.png)

기존 토크나이저는 텍스트를 1차원 숫자 나열(토큰 ID)로 변환합니다.
하지만 이 방식은 "이 단어가 어떤 의미/감정/맥락을 가지는지"까지 담기엔 한계가 있습니다.

RVQ는 데이터를 **2~5차원 이상의 다차원 벡터**로 변환하여, 텍스트뿐 아니라 그 안에 담긴 의미 정보까지 함께 표현합니다.

#### 기존 토크나이저 vs RVQ 양자화

| 항목 | 기존 토크나이저 (1차원) | RVQ 양자화 (다차원) |
|------|:---------------------:|:------------------:|
| **변환 결과** | 토큰 ID 1개 (예: 1024) | 벡터 코드 여러 개 (예: [3, 15, 7, 22]) |
| **담을 수 있는 정보** | 단어 자체 (무슨 글자인지) | 단어 + 의미 + 감정 + 문맥 |
| **차원** | 1차원 | 2~5차원 이상 |
| **적용 대상** | 텍스트만 | 텍스트, 음성, 이미지, 멀티모달 |

#### RVQ 작동 원리

1단계에서 원본 데이터를 가장 가까운 코드북 벡터로 근사하고, 남은 **잔차(Residual)**를 2단계에서 다시 근사합니다. 이를 반복할수록 원본에 가까워집니다.

> 원본 → 1단계 근사 → 잔차 → 2단계 근사 → 잔차 → 3단계 근사 → ...

| 단계 | 역할 | 예시 |
|------|------|------|
| **1단계** | 대략적인 의미 포착 | "이것은 긍정적인 문장이다" |
| **2단계** | 세부 뉘앙스 추가 | "기쁨에 가까운 긍정이다" |
| **3단계** | 미세한 차이 보정 | "흥분된 기쁨이다" |

> 단계가 늘어날수록 더 풍부한 정보를 담을 수 있어, 음성 합성(TTS), 음악 생성, 멀티모달 AI 등에서 핵심적으로 사용됩니다.

---

## 정리

| 주제 | 핵심 |
|------|------|
| **데이터 분리** | 학습/검증/테스트로 나눠야 진짜 성능 측정 가능 |
| **오버피팅** | 학습 데이터를 외우는 현상 → 정규화, 드롭아웃, 조기 종료로 방지 |
| **데이터 변환** | 텍스트 → CSV/DataFrame, 이미지 → RGB 숫자 배열 |
| **토크나이저** | 텍스트를 모델이 이해할 수 있는 토큰(숫자)으로 변환하는 첫 단계 |

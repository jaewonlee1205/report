...
| 토크나이저 | 사용 모델 | 핵심 원리 |
|-----------|----------|-----------|
| **BPE** (Byte Pair Encoding) | GPT 시리즈 | 가장 자주 등장하는 문자 쌍을 반복적으로 병합 |
| **WordPiece** | BERT | BPE와 유사하나, 병합 시 우도(likelihood) 기반으로 선택 |
| **SentencePiece** | T5, LLaMA | 언어에 독립적. 공백도 토큰으로 처리하여 다국어에 강함 |

### 4-4. 토크나이저가 모델 성능에 미치는 영향

같은 문장이라도 토크나이저에 따라 토큰 수가 달라지고, 이는 모델의 입력 길이와 성능에 직접 영향을 줍니다.

| 항목 | 토큰화가 잘 된 경우 | 토큰화가 나쁜 경우 |
|------|:------------------:|:-----------------:|
| 토큰 수 | 적절 (문맥 유지) | 과다 (글자 단위로 쪼개짐) |
| 모델 입력 | 핵심 정보가 보존됨 | 의미 없는 조각으로 분산 |
| 학습 효율 | 빠르고 정확 | 느리고 부정확 |

> 토크나이저는 모델 학습의 첫 단계입니다. 데이터를 아무리 잘 준비해도 토큰화가 잘못되면 모델 성능이 크게 떨어집니다.

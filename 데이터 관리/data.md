...
현실에서는 데이터 샘플이 수백~수천 개밖에 없는 경우가 많습니다.
이럴 때 기존 데이터를 기반으로 **새로운 합성 데이터를 생성**하여 학습 데이터를 늘리는 기법을 사용합니다.

#### SMOTE (Synthetic Minority Over-sampling Technique)

가장 대표적인 테이블 데이터 증강 기법입니다. 소수 클래스의 샘플들 사이를 보간(interpolation)하여 새로운 가짜 샘플을 만들어냅니다.

| 항목 | 내용 |
|------|------|
| **원리** | 소수 클래스 샘플 A와 가장 가까운 이웃 B 사이의 선분 위에 새 데이터를 생성 |
| **효과** | 불균형 데이터(예: 사기 탐지 1% vs 정상 99%)에서 소수 클래스를 늘려 모델 학습 개선 |
| **주의** | 노이즈 데이터까지 증강될 수 있으므로, 이상치 제거 후 적용 권장 |

#### XGBoost + SMOTE 조합이 강력한 이유

**XGBoost**는 적은 데이터에서도 높은 성능을 내는 앙상블 모델입니다.
여기에 SMOTE로 데이터를 증강하면 시너지가 발생합니다.

| 조건 | XGBoost 단독 | XGBoost + SMOTE |
|------|:------------:|:---------------:|
| 원본 1,000건 (양성 50건) | 양성 재현율 ~60% | 양성 재현율 ~85% |
| 원본 5,000건 (양성 250건) | 양성 재현율 ~75% | 양성 재현율 ~90% |

> **핵심**: 데이터가 적고 클래스가 불균형할수록 SMOTE + XGBoost 조합의 효과가 극대화됩니다.
> 단, 증강은 **학습 데이터에만** 적용하고, 테스트 데이터는 절대 증강하지 않습니다.
